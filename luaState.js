...
let luaStatesPtr = [];
let luaStates = [];
let luaState1 = undefined;
let luaState2 = undefined;
...
let luaL_register = undefined;
let luaL_ref = undefined;
let luaL_openlibs = undefined;
// stack manipulation
// get functions
...
const luaB_print = (Lua_state) => {
	let n = lua_gettop(Lua_state);
	let s = "";
	let i = 1;
	for (; i <= n; i++) {
		if (i > 1) {
		...
		if (lua_type(Lua_state, i) == LUA_TSTRING) {
			s += lua_tostring(Lua_state, i);
		} else if (lua_type(Lua_state, i) == LUA_TNIL) {
			s += 'nil';
		} else if (lua_type(Lua_state, i) == LUA_TBOOLEAN) {
			s += lua_toboolean(Lua_state, i) ? 'true' : 'false';
		} else {
			let t = lua_tolstring(Lua_state, i, NULL);
			s += t;
		}
	}
	console.log('luaB_print: ', s);
	return 0;
}

const listGlobals = (Lua_state) => {
	...
	while (lua_next(Lua_state, -2) != LUA_TNIL) {
		...
		console.log('listGlobals: ', key, value);
		lua_pop(Lua_state, 1);
	}
	lua_pop(Lua_state, 1);
...
const hookLuaprint = (Lua_state) => {
	...
	// listGlobals(Lua_state);
	
	
	// lua_getglobal(Lua_state, _G); // [_G]
	let print = Memory.allocUtf8String("print");
	lua_pushvalue(Lua_state, LUA_GLOBALSINDEX); // [_G]
	lua_getfield(Lua_state, LUA_GLOBALSINDEX, print); // [_G, print]
	let originalcFunction = lua_tocfunction(Lua_state, -1);
	console.log('originalcFunction: ', originalcFunction);
	

	let originalPrint = lua_topointer(Lua_state, -1);
	console.log('originalPrint: ', originalPrint);


	
	console.log(hexdump(originalPrint, { offset: 0, length: 0x100, header: true, ansi: true }));
	...
	// hexdump(originalFunction, { offset: 0, length: 0x100, header: true, ansi: true });
	// hexdump(originalFunction, { offset: 0, length: 0x100, header: false, ansi: false });
...
const hookLuaGetTop = async () => {
	...
	Interceptor.attach(lua_gettopPtr, {
		onEnter: function(args) {
			let luaStete = args[0].toInt32();
			if (luaStates.indexOf(luaStete) == -1) {
				luaStates.push(luaStete);
			...
			if (luaStatesPtr.length == 2) {
				luaState2 = args[0];
				// console.log(luaState1, luaState2);
			}
		}
	});
